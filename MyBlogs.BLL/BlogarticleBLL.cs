...
namespace MyBlogs.BLL
{
	...
	{
		private readonly MyBlogs.DAL.BlogarticleDAL dal=new MyBlogs.DAL.BlogarticleDAL();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public MyBlogs.Model.Blogarticle GetModelByCache(int Id)
		{
			
			string CacheKey = "BlogarticleModel-" + Id;
			...
			return (MyBlogs.Model.Blogarticle)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MyBlogs.Model.Blogarticle> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MyBlogs.Model.Blogarticle> DataTableToList(DataTable dt)
		{
			List<MyBlogs.Model.Blogarticle> modelList = new List<MyBlogs.Model.Blogarticle>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MyBlogs.Model.Blogarticle model;
